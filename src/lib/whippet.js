...
function whip() {
	...
	if (config.defaultOtherCategory)
		defaultOtherCategory = config.defaultOtherCategory;
	...
	for (var i = 0; i < includesList.length; i++) {
		...
		var includeContent = fs
			.readFileSync(includesList[i], { encoding: "utf8" })
			.toString();
	...
	var returnMenu = function returnMenu(pageURL, parentPage) {
		...
		if (
			pageURL.indexOf(pagesFolder) !== -1 ||
			pageURL.indexOf(blogFolder) !== -1
		) {
			inFolder = true;
		}
		...
		for (var i = 0; i < menu.menu.length; i++) {
			...
			// mark it as active
			if (
				theMenu[i].url === pageURL ||
				path.basename(theMenu[i].url) === parentPage
			)
				theMenu[i].active = true;
			if (
				pageURL.indexOf(blogFolder) !== -1 &&
				theMenu[i].url.indexOf(blogFolder) !== -1
			)
				theMenu[i].active = true;
	...
	for (var i = 0; i < pagemds.length; i++) {
		...
		if (shouldParse) {
			...
			if (!pageVars.template || pageVars.template.length === 0) {
				...
				if (pagemds === "index.md")
					pageVars.template = defaultIndexTemplate;
			}
			...
			if (pagemds[i].split(path.sep)[0] === "blog") {
				if (!pageVars.intro)
					pageVars.intro = pageVars.content.substring(
						0,
						config.introLength || 500
					);
			}

			// mustache parse two (template)
			var template = fs
				.readFileSync(templatesFolder + "/" + pageVars.template, {
					encoding: "utf8",
				})
				.toString();
			...
			var outFile =
				"site/" +
				pagemds[i].substring(0, pagemds[i].lastIndexOf(".")) +
				".html";
	...
	if (rebuildBlogIndexes) {
		...
		var archiveMaker = (function archiveMaker() {
			...
			var fillArchives = function fillArchives(year, month) {
				for (var i = 0; i < blogPosts.length; i++) {
					if (
						blogPosts[i].postDate.getFullYear() === year &&
						blogPosts[i].postDate.getMonth() + 1 === month
					) {
		...
		var archivePageMaker = (function archivePageMaker() {
			...
			for (var i in archives) {
				...
				indexData.menu = returnMenu(
					blogFolder + "/" + indexData.archiveUrl
				);

				// read the template
				var template = fs
					.readFileSync(
						templatesFolder + "/" + defaultBlogArchiveTemplate,
						{ encoding: "utf8" }
					)
					.toString();
				...
				siteMapPages.push({
					title:
						"Blog Archive: " +
						indexData.archiveMonthString +
						", " +
						indexData.archiveYear,
		...
		var categoryPageMaker = (function categoryPageMaker() {
			for (var i in categoriesArray) {
				...
				indexData.menu = returnMenu(
					blogFolder + "/" + indexData.categoryUrl
				);

				// read the template
				var template = fs
					.readFileSync(
						templatesFolder + "/" + defaultBlogCategoryTemplate,
						{ encoding: "utf8" }
					)
					.toString();
		...
		var createIndex = function createIndex(index, blogPosts) {
			...
			// read the template
			var template = fs
				.readFileSync(templatesFolder + "/" + defaultBlogIndexTemplate, {
					encoding: "utf8",
				})
				.toString();
	...
	// read the template
	var siteMapTemplate = fs
		.readFileSync(templatesFolder + "/" + defaultSitemapTemplate, {
			encoding: "utf8",
		})
		.toString();
	...
	// save it
	var sitemapOutFile =
		"site/" + defaultSitemapTemplate.replace(".html", ".xml");
